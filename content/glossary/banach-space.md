...
A **Banach space** is a {{< knowl id="vector-space" text="vector space" >}} $V$ equipped with a {{< knowl id="norm" text="norm" >}} $\|\cdot\|$ such that $V$ is **complete** with respect to the induced {{< knowl id="metric" text="metric" >}} $d(\mathbf{u}, \mathbf{v}) = \|\mathbf{u} - \mathbf{v}\|$.
